...
class TrainDatasetStage1(torch.utils.data.Dataset):
    def __init__(self, args, dataset_folder, M=10, alpha=30, N=5, L=2,
                 current_group=0, min_images_per_class=10):
        super().__init__()
        self.M = M
        ...
        # 初始化图片路径列表和图片到类别ID的映射
        self.all_images = []
        self.image_to_class_id = {}

        for class_id in self.classes_ids:
            for img_path in self.images_per_class[class_id]:
                self.all_images.append(os.path.join(self.dataset_folder, img_path))
                self.image_to_class_id[os.path.join(self.dataset_folder, img_path)] = class_id
        ...
        if self.augmentation_device == "cpu":
            self.transform = T.Compose([
                T.ColorJitter(brightness=args.brightness,
                              ...
                              saturation=args.saturation,
                              hue=args.hue),
                T.RandomResizedCrop([args.image_size, args.image_size], scale=[1 - args.random_resized_crop, 1],
                                    antialias=True),
                T.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
            ])
    ...
    def __getitem__(self, index):
        ...
        # 使用反向映射字典来找到图片的类别ID
        class_id = self.image_to_class_id[image_path]

        # 将全局类别ID转换为当前组的类别索引
        class_num = self.classes_ids.index(class_id)
    ...
    def get_classes_num(self):
        """Return the number of classes in the current group."""
        return len(self.classes_ids)
